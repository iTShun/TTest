...
{
	Logger_Assert(NULL != _dst, "_dst can't be NULL!");
	Logger_Assert(NULL != _src, "_src can't be NULL!");
	Logger_Assert(0 < _dstSize, "_dstSize can't be 0!");
...
int32_t vsnwprintf(wchar_t* _out, int32_t _max, const wchar_t* _format, va_list _argList)
{
	...
	int32_t total = 0;
#if CRT_NONE
	UNUSED(_out, _max, _format, argList);
#elif CRT_MSVC
	int32_t len = -1;
	if (NULL != _out)
	{
